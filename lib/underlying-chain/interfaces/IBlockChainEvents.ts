...
export interface IBlockChainEvents {
    ...
    // Filter fields are implementation dependent, but at least `hash` (transaction hash), `from`, `to` (string, matching any od inputs/outputs in transaction) and `reference` (string '0x...', matching payment reference) should be supported.
    // Returns subscriptionId (string), used for unsubscribing.
