...
contract(`AssetManagerSimulation.sol; ${getTestFile(__filename)}; Asset manager simulations`, async accounts => {
    ...
    describe("simple scenarios - price change liquidation", () => {
        ...
        it("ccb due to price change (no liquidation due to collateral deposit)", async () => {
            ...
            assert.isTrue(ccb);
            const info = await agent.checkAgentInfoOld(fullAgentCollateral, crt.feeUBA, crt.valueUBA, minted.mintedAmountUBA, 0, 0, 0, 1);
            ...
            const info2 = await agent.checkAgentInfoOld(fullAgentCollateral.add(additionalCollateral), crt.feeUBA, crt.valueUBA, minted.mintedAmountUBA, 0, 0, 0, 0);
        ...
        it("ccb due to price change (no liquidation due to partial self close)", async () => {
            ...
            const minted = await minter.executeMinting(crt, txHash);
            assertWeb3Equal(minted.mintedAmountUBA, await context.convertLotsToUBA(lots));
            await agent.checkAgentInfoOld(fullAgentCollateral, crt.feeUBA, crt.valueUBA, minted.mintedAmountUBA, 0, 0);
            // price change
            await context.natFtso.setCurrentPrice(6, 0);
            await context.assetFtso.setCurrentPrice(toBNExp(10, 6), 0);
            // start ccb
            ...
            assert(collateralRatioBIPS.gte(toBN((await context.assetManager.getSettings()).minCollateralRatioBIPS)));
            assert(collateralRatioBIPS.lt(toBN((await context.assetManager.getSettings()).safetyMinCollateralRatioBIPS)));
            const info2 = await agent.checkAgentInfoOld(fullAgentCollateral, crt.feeUBA.add(selfClosedValueUBA), crt.valueUBA.sub(selfClosedValueUBA), minted.mintedAmountUBA.sub(selfClosedValueUBA), 0, 0, 0, 0);
            assertWeb3Equal(info2.ccbStartTimestamp, 0);
            assertWeb3Equal(info2.liquidationStartTimestamp, 0);
            // agent "buys" f-assets
        ...
        it("liquidation due to price change (agent can be safe again)", async () => {
            ...
            const info = await agent.checkAgentInfoOld(fullAgentCollateral.sub(liquidationReward1), crt.feeUBA.add(liquidatedUBA1), crt.valueUBA.sub(liquidatedUBA1), minted.mintedAmountUBA.sub(liquidatedUBA1), 0, 0, 0, 2);
            ...
            const info2 = await agent.checkAgentInfoOld(fullAgentCollateral.sub(liquidationReward1).sub(liquidationReward2), crt.feeUBA.add(liquidatedUBA1).add(liquidatedUBA2), crt.valueUBA.sub(liquidatedUBA1).sub(liquidatedUBA2), minted.mintedAmountUBA.sub(liquidatedUBA1).sub(liquidatedUBA2));
            assertWeb3Equal(info2.ccbStartTimestamp, 0);
            assertWeb3Equal(info2.liquidationStartTimestamp, 0);
            // final tests
        ...
        it("liquidation due to price change (agent cannot be safe again)", async () => {
            ...
            const liquidationReward2 = await liquidator.getLiquidationReward(liquidatedUBA2, liquidationFactorBIPS2);
            assertWeb3Equal(endBalanceLiquidator2.sub(startBalanceLiquidator2), liquidationReward2);
            const info2 = await agent.checkAgentInfoOld(fullAgentCollateral.sub(liquidationReward1).sub(liquidationReward2), crt.feeUBA.add(liquidatedUBA1).add(liquidatedUBA2), crt.valueUBA.sub(liquidatedUBA1).sub(liquidatedUBA2), minted.mintedAmountUBA.sub(liquidatedUBA1).sub(liquidatedUBA2), 0, 0, 0, 2);
            assertWeb3Equal(info2.ccbStartTimestamp, 0);
            ...
            const info3 = await agent.checkAgentInfoOld(fullAgentCollateral.sub(liquidationReward1).sub(liquidationReward2).sub(liquidationReward3), crt.feeUBA.add(crt.valueUBA), 0, 0);
        ...
        it("liquidation due to price change (agent can end liquidation after new price change)", async () => {
            ...
            const info1 = await agent.checkAgentInfoOld(fullAgentCollateral.sub(liquidationReward1), crt.feeUBA.add(liquidatedUBA1), crt.valueUBA.sub(liquidatedUBA1), minted.mintedAmountUBA.sub(liquidatedUBA1), 0, 0, 0, 2);
            ...
            // final tests
            const info2 = await agent.checkAgentInfoOld(fullAgentCollateral.sub(liquidationReward1), crt.feeUBA.add(liquidatedUBA1), crt.valueUBA.sub(liquidatedUBA1), minted.mintedAmountUBA.sub(liquidatedUBA1));
            assertWeb3Equal(info2.ccbStartTimestamp, 0);
            assertWeb3Equal(info2.liquidationStartTimestamp, 0);
            const collateralRatioBIPS2 = await agent.getCollateralRatioBIPS(fullAgentCollateral.sub(liquidationReward1), minted.mintedAmountUBA.sub(liquidatedUBA1));
