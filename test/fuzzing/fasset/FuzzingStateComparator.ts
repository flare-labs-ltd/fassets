...
export class FuzzingStateComparator {
    ...
    checkEquality(description: string, actualValue: unknown, trackedValue: unknown, options?: CheckNumericOptions) {
        if (this.isNumeric(actualValue) && this.isNumeric(trackedValue)) {
            ...
            return this.checkStringEquality(description, actualValue, trackedValue, options?.alwaysLog ?? false);
        }
    }
    ...
    checkNumericDifference(description: string, actualValue: BNish, comparison: 'eq' | 'lte' | 'gte', trackedValue: BNish, options?: CheckNumericOptions) {
